...
func (c *Config) Validate() error {
	...
	if c.JiraAPIToken == "" {
		return errors.New("missing Jira API token (JIRA_API_TOKEN)")
	}
...
func LoadConfig() *Config {
	return &Config{
		...
		TeamMembers:             parseCSV(getenv("TEAM_MEMBERS", "")),
		ProcessedLabel:          getenv("PROCESSED_LABEL", "triage-agent-done"),
		PollingInterval:         parseDurationOrDefault(getenv("POLLING_INTERVAL", "1h"), time.Hour),
