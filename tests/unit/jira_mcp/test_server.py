"""Tests for jira-mcp MCP server core functionality."""
...
# Add jira-mcp src to Python path
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / "jira-mcp" / "src"))
    ...
    def test_get_tool_config(self) -> None:
        ...
        with patch("core.utils.load_config") as mock_load:
            mock_load.return_value = {
                "tools": {
                    "echo": {"prefix": "[TEST] "},
                    "weather": {"api_key_env": "WEATHER_API_KEY"}  # pragma: allowlist secret
                }
            }
            ...
            assert weather_config["api_key_env"] == "WEATHER_API_KEY"  # pragma: allowlist secret
